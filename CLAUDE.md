...
folder (`FileTree.onChangeRoot`), `⌘Backspace` to the parent. Re-rooting into a
git repo runs `git pull --ff-only` in the background (`pullIfRepo`). `⌘⇧C`
opens the clone dialog (`components/clone.js`).
